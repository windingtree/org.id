    ...
    switch (model[key].type) {
      case 'enum':
        ...
        if (!model[key].values || !Array.isArray(model[key].values)) {
          ...
          throw new ExpectError('Enumerator conditions array not defined in the model', {
            expected: 'enum',
        ...
        if (!model[key].values.includes(value) &&
          ...
          throw new ExpectError(`The value type of the "${String(key)}" property is not valid. Expected type one of ${model[key].values} but got: ${String(value)}`, {
            expected: 'enum',
            values: model[key].values,
            key,
            value
      ...
      case 'address':
        ...
        if (!new RegExp('^0x[a-fA-F0-9]{40}$').test(value) &&
          ...
          throw new ExpectError(`Ethereum address is required as value for the property: "${key}"`, {
            expected: 'address',
            key,
            value
          });
      ...
      case 'hash':
        ...
        if (!new RegExp('^0x[a-fA-F0-9]{64}$').test(value) &&
          ...
          throw new ExpectError(`Ethereum tx hash is required as value for the property: "${key}"`, {
            expected: 'hash',
            key,
            value
          });
      ...
      case 'bn':
        ...
        if (!web3utils.isBN(value) &&
          ...
          throw new ExpectError(`BN instance expected as value for the property "${key}" but got: ${value}`, {
            expected: 'bn',
            key,
            value
          });
      ...
      default:
        ...
        if (typeof value !== model[key].type &&
          ...
          throw new ExpectError(`The value type of the "${key}" property is not valid: ${value}`, {
            expected: model[key].type,
            key,
            value
          });
