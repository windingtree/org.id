...
contract('ORG.ID', accounts => {
    ...
    describe('ORG.ID methods', () => {
        ...
        describe('#setOrgJson(bytes32,string,bytes32)', () => {
            ...
            describe('Changing Uri and Hash by unit director', () => {
                ...
                it('should fail if called by non-owner or director', async () => {
                    await assertRevert(
                        orgIdContract
                            .methods['setOrgJson(bytes32,string,bytes32)'](
                                unitOrgIdHash,
                ...
                it('should change ORG.JSON URI and hash if called by confirmed director', async () => {
                    await orgIdContract
                        .methods['confirmDirectorship(bytes32)'](unitOrgIdHash)
        ...
        describe('#getOrganizations(bool)', () => {
            ...
            describe('with incative organizations', () => {
                ...
                beforeEach(async () => {
                    orgId1 = await createOrganizationHelper(
                        orgIdContract,
                        randomAddress,
                        [mockOrgJsonUri, mockOrgJsonHash]
                    );
                    orgId2 = await createOrganizationHelper(
                        orgIdContract,
                        randomAddressTwo,
                        [mockOrgJsonUri, mockOrgJsonHash]
                    );
    ...
    describe('ORG.ID hierarchy', () => {
        ...
        describe('#createUnit(bytes32,address,string,bytes32)', () => {
            ...
            it('should create a unit if director address is zero', async () => {
        ...
        describe('#confirmDirectorship(bytes32)', () => {
            let unitOrgIdHash;
            ...
            it('should fail if organization not found', async () => {
                await assertRevert(
                    orgIdContract
                        .methods['confirmDirectorship(bytes32)'](zeroHash)
                        .send({ from: unitDirector }),
                    'ORG.ID: Organization not found'
                );
            });
        ...
        describe('#transferDirectorship(bytes32,address)', () => {
            ...
            it('should confirm directorship if transferred to owner', async () => {
                const result = await orgIdContract
                    .methods['transferDirectorship(bytes32,address)'](
                        unitOrgIdHash,
                        unitOwner
        ...
        describe('#getUnits(bytes32,bool)', () => {
            ...
            describe('with incative units', () => {
                let unit1;
                let unit2;

                beforeEach(async () => {
                    unit1 = await createUnitHelper(
                        orgIdContract,
                        parentOrgIdOwner,
                        [
                            parentOrgIdHash,
                    ...
                    await orgIdContract.methods['toggleOrganization(bytes32)'](unit1)
                        .send({ from: parentOrgIdOwner });
                    await orgIdContract.methods['toggleOrganization(bytes32)'](unit2)
                        .send({ from: parentOrgIdOwner });
                });

                it('should return empty array if registry contains only inactive units', async () => {
                    const orgs = await orgIdContract
                        .methods['getUnits(bytes32,bool)'](parentOrgIdHash, false)
                        .call();
                    (orgs).should.to.be.an('array');
                ...
                it('should return array which includes inactive units', async () => {
                    const orgs = await orgIdContract
